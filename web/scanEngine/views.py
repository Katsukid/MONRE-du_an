...
from startScan.models import SASTScan
...
@has_permission_decorator(PERM_MODIFY_FILE_UPLOAD, redirect_url=FOUR_OH_FOUR_URL)
def file_list(request, slug):
    files = UploadFile.objects.filter(project__slug=slug).order_by('id')
    context = {
            'scan_engine_nav_active': 'active',
            'file_li': 'active',
            'files': files}
    return render(request, 'scanEngine/SAST/index.html', context)
...
def add_file(request, slug):
    ...
    context['form'] = form
    return render(request, 'scanEngine/SAST/add.html', context)
...
@has_permission_decorator(PERM_MODIFY_FILE_UPLOAD, redirect_url=FOUR_OH_FOUR_URL)
def delete_file(request, slug, id):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.INFO,
            'File upload successfully deleted!')
    else:
        responseData = {'status': 'false'}
        messages.add_message(
            request,
            messages.ERROR,
            'Oops! File upload could not be deleted!')
    return http.JsonResponse(responseData)
...
@has_permission_decorator(PERM_MODIFY_FILE_UPLOAD, redirect_url=FOUR_OH_FOUR_URL)
def scan_file(request, slug, id):
    obj = get_object_or_404(UploadFile, id=id)
    if request.method == "POST":
        # Thư mục đích để giải nén
        extract_path = f'/usr/src/app/file_upload/{obj.short_name}'
        try:
            # Đường dẫn đến file .tar
            tar_file_path = f'/usr/src/app/file_upload/{obj.short_name}.tar'
            ...
            with tarfile.open(tar_file_path, 'r') as tar:
                ...
                # Thực thi lệnh semgrep và lưu kết quả
                scan_command = ['semgrep', '--config', f'/usr/src/app/semgrep-rule.yml', f'/usr/src/app/file_upload/{obj.short_name}', '--json']
                result = subprocess.run(scan_command, capture_output=True, text=True, check=True)

                # Xử lý kết quả
                responseData = {'status': True, 'output': json.loads(result.stdout), 'id': obj.id}
                # print(responseData)
                SASTScan.objects.create(filename=obj, results=responseData['output'], scan_status=2, start_scan_date=timezone.now())
                messages.add_message(request, messages.SUCCESS, 'Scan completed successfully!')

        except tarfile.TarError as e:
            responseData = {'status': False, 'error': f"Error extracting tar file: {str(e)}"}
            messages.add_message(request, messages.ERROR, f'Tar extraction failed: {str(e)}')
        except subprocess.CalledProcessError as e:
            responseData = {'status': False, 'error': e.stderr}
            # print(responseData)
            SASTScan.objects.create(filename=obj, results=responseData['error'], scan_status=0, start_scan_date=timezone.now())
            messages.add_message(request, messages.ERROR, 'Scan failed. Please check the logs.')
        except Exception as e:
            responseData = {'status': False, 'error': str(e)}
            print(responseData)
            messages.add_message(request, messages.ERROR, f'An unexpected error occurred: {str(e)}')
        finally:
            # Xóa thư mục đã giải nén, bất kể quét thành công hay thất bại
            if os.path.exists(extract_path):
                shutil.rmtree(extract_path)
                print(f"Deleted extracted folder: {extract_path}")
        # return http.HttpResponseRedirect(reverse('sastscan_history', kwargs={'slug': slug}))    
        return http.JsonResponse(responseData)
    # return http.JsonResponse({'status': False, 'error': 'Invalid request method'})
