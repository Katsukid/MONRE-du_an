...
class ThemNenTangApi(ListCreateAPIView):
    model = NenTangPhatTrien
    serializer_class = NenTangSerializer

    def create(self, request, *args, **kwargs):
        serializer = NenTangSerializer(data=request.data)

        if serializer.is_valid():
            serializer.save()

            return JsonResponse({
                'message': 'Thêm mới nền tảng thành công!'
            }, status=status.HTTP_201_CREATED)  # Đổi status thành 201 Created cho thành công
        else:
            return JsonResponse({
