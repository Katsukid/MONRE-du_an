...
def sastscan_history(request, slug):
    try:
        sastscan = SASTScan.objects.filter(filename__project__slug=slug).order_by('-start_scan_date')
        context = {'scan_history_active': 'active', "sastscan": sastscan}
        return render(request, 'startScan/sastscan_history.html', context)
    except Exception as e:
        print(f"Error in sastscan_history: {str(e)}")
...
def sast_detail_scan(request, id, slug):
    try:
        context = {}
        sastscan = get_object_or_404(SASTScan, id=id)
        # Chuyển đổi trường results thành đối tượng Python
        results = json.loads(sastscan.results) if isinstance(sastscan.results, str) else sastscan.results
        # Tinh toan so loi co cung muc severity
        info_count = sum(1 for result in results.get('results', []) 
                            if result.get('extra', {}).get('severity') == 'INFO')
        warning_count = sum(1 for result in results.get('results', []) 
                            if result.get('extra', {}).get('severity') == 'WARNING')
        error_count = sum(1 for result in results.get('results', []) 
                            if result.get('extra', {}).get('severity') == 'ERROR')
        total_count = sum(1 for result in results.get('results', []) 
                            if result.get('check_id'))
        # Lay mot truong bat ky trong co so du lieu luu bang json
        vulnerabilities = []
        for result in results.get('results', []):
            vulnerability = {
                'cwe': result.get('extra', {}).get('metadata', {}).get('cwe', []),
                'technology': result.get('extra', {}).get('metadata', {}).get('technology', []),
                'scantime': sastscan.start_scan_date,
                'severity': result.get('extra', {}).get('severity'),
                'path': result.get('path'),
                'line': result.get('end', {}).get('line'),
        ...
        return render(request, 'startScan/sast_detail_scan.html', context)
    except Exception as e:
        print(f"Error in sast_detail_scan: {str(e)}")
    ...
    if request.method == "POST":
        for key, value in request.POST.items():
            if key == 'scan_history_table_length' or key == 'csrfmiddlewaretoken':
                continue
            scan = get_object_or_404(ScanHistory, id=value) # lay doi tuong ra de xoa cac ban ghi co lien quan o cac bang khac
...
@has_permission_decorator(PERM_MODIFY_SCAN_RESULTS, redirect_url=FOUR_OH_FOUR_URL)
def delete_sastscans(request, slug):
    try:
        if request.method == "POST":
            for key, value in request.POST.items():
                if key == 'sastscan_history_table_length' or key == 'csrfmiddlewaretoken':
                    continue
                SASTScan.objects.filter(id=value).delete() # chi xoa o mot bang
            messages.add_message(
                request,
                messages.INFO,
                'All Scans deleted!')
        return HttpResponseRedirect(reverse('sastscan_history', kwargs={'slug': slug}))
    except Exception as e:
        print(f"Error in delete sastscan_history: {str(e)}")
        return HttpResponse(f"An error occurred: {str(e)}")
